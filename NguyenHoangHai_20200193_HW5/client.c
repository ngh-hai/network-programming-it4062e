// When uploading the file to server, the client needs to sleep for 2 seconds.
// This makes sure the EOF signal is not concatenated to the content of the file.
...
int main(int argc, char *argv[]) {
    ...
    while (true) {
        ...
        if (client_status == SELECT_IN_PROGRESS) {
            ...
            buff[strlen(buff) - 1] = '\0';
            FILE *f = fopen(buff, "r");
            if (f == NULL) { // error in opening file
                perror("fopen");
                client_status = SELECT_IN_PROGRESS;
                choice = -1;
                strcpy(buff, "");
            ...
            sleep(2); // make sure EOF signal does not be concatenated to the file content.
            // EOF
            strcpy(buff, "");
            bytes_sent = send(client_sock, buff, 1, 0);
            if (bytes_sent <= 0) {
