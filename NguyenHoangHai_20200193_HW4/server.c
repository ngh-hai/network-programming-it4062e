...
int main(int argc, char *argv[]) {
    ...
    while(true) {
        ...
        bytes_received = recvfrom(server_sock, buff, BUFF_SIZE, 0, (struct sockaddr *) &server, (socklen_t*)&sin_size);
        if(bytes_received < 0) {
            perror("\nError: ");
            return 0;
        }
        ...
        if(message_status == 0) {
            bytes_sent = sendto(server_sock, buff, strlen(buff), 0, (struct sockaddr *) &server, sizeof(struct sockaddr));
            if(bytes_sent < 0) {
                perror("\nError: ");
                return 0;
            }
            ...
            bytes_sent = sendto(server_sock, digits, strlen(digits), 0, (struct sockaddr *) &server, sizeof(struct sockaddr));
            if(bytes_sent < 0) {
                perror("\nError: ");
                return 0;
            }
            bytes_sent = sendto(server_sock, letters, strlen(letters), 0, (struct sockaddr *) &server, sizeof(struct sockaddr));
            if(bytes_sent < 0) {
                perror("\nError: ");
                return 0;
            }
